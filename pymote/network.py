...
class Network(Graph):
    ...
    def show(self, savefig='', positions=None, edgelist=None, nodeColor='r',\
        ...
        nx.draw_networkx_nodes(net,pos,node_size=node_size,node_color=nodeColor,cmap='YlOrRd')
        if (show_labels):
            label_pos = {}
            for n in net.nodes():
    ...
    def send(self, destination, message):
        ...
        if destination in self.nodes():
            ...
            raise PymoteMessageUndeliverable('Destination node not in network.',
                                             message)
        
    def get_dic(self):
        """ Return all network data in form of dictionary. """
        algorithms = {'%d %s' % (ind,algorithm.name): 'active' if algorithm==self.algorithms[self.algorithmState['index']] else '' 
                      for ind,algorithm in enumerate(self.algorithms)}
        pos = {node:'x: %.2f y: %.2f theta: %.2f deg' % 
               (self.pos[node][0],self.pos[node][1],self.ori[node]*180./pi) 
               for node in self.nodes()}
    ...
    def get_tree_net(self, treeKey):
        ...
        while nodesToCheck:
            ...
            edgelist +=  [(node,child) for child in node.memory[treeKey]['children'] if child in self.nodes()]
            levels[self.nodes().index(node)] = level  
            nodesToCheck += [(child,level+1) for child in node.memory[treeKey]['children'] if child in self.nodes()]
