...
class Environment2D(Environment):
    ...
    def are_visible(self,xy0,xy1):
        """ Returns true if source (x0,y0) and destination (x1,y1) are 'visible'.
        ...
        # if float point (x,y) is on N or E edge then check also W or S neighbor
        while (not((int(x)==int(x1) or (x%1==0 and int(x)-1==int(x1))) and 
                   (int(y)==int(y1) or (y%1==0 and int(y)-1==int(y1))))):
            if incrE>0:
                dx = 1-x%1
            else:
