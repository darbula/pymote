...
from numpy import array, sqrt, dot, outer, concatenate, set_printoptions, arctan2, sin, cos, nan
from numpy.linalg import eig, det
from numpy.lib.twodim_base import eye
from pymote.utils.localization.stitchsubclusterselectors import MaxCommonNodeSelector
...
class DistStitcher(object):
        ...
        while True:
            ...
            for node in src[srcSubIndex].keys():
                if not node in dst[dstSubIndex]: # append only new
                    dst[dstSubIndex][node] = concatenate((t + dot(R,src[srcSubIndex][node][:2]),[nan]))
    ...
    def align(self, dst, src):
        ...
        for srcSubIndex in range(len(src)):
            R, s, t = self.stitch_method(dst[0],src[srcSubIndex]) #@UnusedVariable
            for node in src[srcSubIndex].keys():
                src[srcSubIndex][node] = concatenate((t + dot(R,src[srcSubIndex][node][:2]),[nan]))
    ...
    def stitch_subclusters_horn(self, dstSubPos, srcSubPos):
        ...
        if len(commonNodes)==1:
            R = eye(2)
        elif len(commonNodes)==2: # for 2 commonNodes R is calculated directly 
