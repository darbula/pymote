...
from numpy import asarray, array, sqrt, dot, outer, concatenate, set_printoptions, arctan2, sin, cos, pi, mod, isnan, trace, zeros, ones, eye, vstack, inf, diag, mean
...
def get_rms(truePos, estimated, align=False, scale=False):
    ...
    for estimated_subcluster in estimated:
        for node in estimated_subcluster:
            suma += (estimated_subcluster[node][0]-truePos[0][node][0])**2 +\
            (estimated_subcluster[node][1]-truePos[0][node][1])**2
            node_count += 1
...
def show_localized(net, estimated, scale=False, align=True,\
    ...
    if len(estimated)>1:
        ...
        if display_loc_err:
            ...
            edge_pos = asarray([(net.pos[n],estimated_sc[n]) for n in estimated_sc.keys()])
